...
const HomePage = () => {
  ...
  // Update search query whenever searchTerm changes
  useEffect(() => {
  ...
  const handleVoiceSearch = () => {
    ...
    recognition.onresult = (event: any) => {
      ...
      setSearchTerm(transcript); // Update search input
      setSearchQuery(transcript); // Trigger filtering
      setListening(false);
    };
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center z-20">
          ...
          >
            ...
            <button
              ...
              className={`p-2 rounded-full transition-colors ${listening ? 'bg-red-200' : 'hover:bg-gray-100'}`}
            >
              <Mic className={`w-4 h-4 ${listening ? 'text-red-500' : 'text-gray-500'}`} />
            </button>
          </motion.div>
        </div>
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex gap-8">
            <FilterSidebar showMobile={showFilters} onClose={() => setShowFilters(false)} onFiltersChange={setFilters} />
            ...
            <div className="flex-1">
              ...
              {filteredProducts.length === 0 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
                  <p className="text-gray-500 text-lg">No products match your current search or filter criteria.</p>
                </motion.div>
              )}
            </div>
          </div>
